...
if SERVER then
  ...
  if not GlobalState.vrp_tunnel then
    GlobalState.vrp_tunnel = lib.string.random('_TAAA1AA111A_')
  end
else
  ...
  local fcall = function(...)
    ...
    if delay_data[2] > 0 then -- delay trigger
      SetTimeout(delay_data[2], function()
        ...
        if SERVER then
          TriggerRemoteEvent(iname .. GlobalState.vrp_tunnel, dest, fname, args, identifier, rid)
        else
          TriggerRemoteEvent(iname .. GlobalState.vrp_tunnel , fname, args, identifier, rid)
        end
      end)
    else -- no delay
      -- send request
...
function Tunnel.bindInterface(name, interface)
  -- receive request
  RegisterLocalEvent(name .. GlobalState.vrp_tunnel , function(member, args, identifier, rid)
    local source = source
    ...
    if rid >= 0 then
      if SERVER then
        TriggerRemoteEvent(name .. ":" .. identifier .. GlobalState.vrp_tunnel, source, rid, rets)
      else
        TriggerRemoteEvent(name .. ":" .. identifier .. GlobalState.vrp_tunnel, rid, rets)
      end
    end
  end)
end
...
function Tunnel.getInterface(name, identifier)
  ...
  -- receive response
  RegisterLocalEvent(name .. ":" .. identifier .. GlobalState.vrp_tunnel, function(rid, args)
