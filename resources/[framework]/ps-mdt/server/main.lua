...
local FivemerrApiKey = GetConvar('ps-mdt:fivemerr:api', '')
...
lib.callback.register('ps-mdt:server:MugShotWebhook', function()
	...
	elseif Config.FivemerrMugShot then
		if FivemerrMugShot == '' then
			...
			return FivemerrMugShot, FivemerrApiKey
		end
	else
...
if Config.UseWolfknightRadar == true then
	...
	AddEventHandler("wk:onPlateScanned", function(cam, plate, index)
		...
		local driversLicense = PlayerData.datatable['licences'].driver

		if bolo == true then
			TriggerClientEvent('QBCore:Notify', src,
				'BOLO ID: ' .. boloId .. ' | Title: ' ..
				title .. ' | Registered Owner: ' .. vehicleOwner .. ' | Plate: ' .. plate, 'error',
				Config.WolfknightNotifyTime)
		end
		if warrant == true then
			TriggerClientEvent('QBCore:Notify', src,
				'WANTED - INCIDENT ID: ' .. incidentId .. ' | Registered Owner: ' .. owner .. ' | Plate: ' .. plate,
				'error', Config.WolfknightNotifyTime)
		end

		if Config.PlateScanForDriversLicense and driversLicense == false and vehicleOwner then
			TriggerClientEvent('QBCore:Notify', src,
				'NO DRIVERS LICENCE | Registered Owner: ' .. vehicleOwner .. ' | Plate: ' .. plate, 'error',
				Config.WolfknightNotifyTime)
		end
...
-- 	local src = source
-- 	local player = vRP.getPlayerInfo( src )
-- 	if GetActiveData(player.PlayerData.id) then
-- 		activeUnits[player.PlayerData.id] = nil
-- 	end
-- end)
...
AddEventHandler('playerDropped', function(reason)
	...
	local firstName = PlayerData.firstname:sub(1, 1):upper() .. PlayerData.firstname:sub(2)
	local lastName = PlayerData.lastname:sub(1, 1):upper() .. PlayerData.lastname:sub(2)

	-- Auto clock out if the player is off duty
	if IsPoliceOrEms(job) and PlayerData.job.onduty then
		MySQL.query.await(
			'UPDATE mdt_clocking SET clock_out_time = NOW(), total_time = TIMESTAMPDIFF(SECOND, clock_in_time, NOW()) WHERE user_id = @user_id ORDER BY id DESC LIMIT 1',
			{
		...
		if result then
			...
			sendToDiscord(16711680, "MDT Clock-Out",
				'Player: **' ..
				firstName ..
				" " ..
				lastName ..
				...
				PlayerData.job.name ..
				'**\n\nRank: **' ..
				PlayerData.job.rankName .. '**\n\nStatus: **Off Duty**\n Total time:' .. time_formatted,
				"ps-mdt | Made by Project Sloth")
		end
	end

	-- Delete player from the MDT on logout
	if PlayerData ~= nil then
		if GetActiveData(PlayerData.id) then
			activeUnits[PlayerData.id] = nil
		end
	else
...
RegisterNetEvent("ps-mdt:server:ToggleDuty", function()
	...
	if not xplayer.job.onduty then
		--// Remove from MDT
		if GetActiveData(xPlayer.id) then
			activeUnits[xPlayer.id] = nil
		end
	end
end)


RegisterCommand('mdtleaderboard', function (source)
	...
	local result = MySQL.Sync.fetchAll(
		'SELECT firstname, lastname, total_time FROM mdt_clocking ORDER BY total_time DESC')
	...
	for i, record in ipairs(result) do
		...
		leaderboard_message = leaderboard_message ..
			i .. '. **' .. firstName .. ' ' .. lastName .. '** - ' .. total_time .. '\n'
	end
...
RegisterNetEvent("ps-mdt:server:ClockSystem", function()
	...
	if PlayerData.job.onduty then
		...
		MySQL.Async.insert(
			'INSERT INTO mdt_clocking (user_id, firstname, lastname, clock_in_time) VALUES (:user_id, :firstname, :lastname, :clock_in_time) ON DUPLICATE KEY UPDATE user_id = :user_id, firstname = :firstname, lastname = :lastname, clock_in_time = :clock_in_time',
			{
				user_id = PlayerData.id,
				firstname = firstName,
				lastname = lastName,
		...
		sendToDiscord(65280, "MDT Clock-In",
			'Player: **' ..
			firstName ..
			" " ..
			lastName ..
			'**\n\nJob: **' ..
			PlayerData.job.name .. '**\n\nRank: **' .. PlayerData.job.rankName .. '**\n\nStatus: **On Duty**',
			"ps-mdt | Made by Project Sloth")
	else
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	activeUnits[tostring(PlayerData.id)] = {
		cid = PlayerData.id,
		callSign = PlayerData.datatable['callsign'],
		firstName = PlayerData.firstname:sub(1, 1):upper() .. PlayerData.firstname:sub(2),
		lastName = PlayerData.lastname:sub(1, 1):upper() .. PlayerData.lastname:sub(2),
		radio = Radio,
	...
	if xPlayer then
		...
		if JobType ~= nil then
			local people = MySQL.query.await(
				"SELECT p.citizenid, p.charinfo, md.pfp, md.fingerprint FROM players p LEFT JOIN mdt_data md on p.citizenid = md.cid WHERE LOWER(CONCAT(JSON_VALUE(p.charinfo, '$.firstname'), ' ', JSON_VALUE(p.charinfo, '$.lastname'))) LIKE :query OR LOWER(`charinfo`) LIKE :query OR LOWER(`citizenid`) LIKE :query OR LOWER(md.fingerprint) LIKE :query AND jobtype = :jobtype LIMIT 20",
				{ query = string.lower('%' .. sentData .. '%'), jobtype = JobType })
			local citizenIds = {}
			...
			if next(convictions) then
				for _, conv in pairs(convictions) do
					...
					people[citizenIdIndexMap[conv.cid]].convictions = people[citizenIdIndexMap[conv.cid]].convictions +
						#charges
				end
			end
...
lib.callback.register('mdt:server:OpenDashboard', function(source, cb)
	local PlayerData = GetPlayerData(source)
...
RegisterNetEvent('mdt:server:NewBulletin', function(title, info, time)
	...
	local newBulletin = MySQL.insert.await(
		'INSERT INTO `mdt_bulletin` (`title`, `desc`, `author`, `time`, `jobtype`) VALUES (:title, :desc, :author, :time, :jt)',
		{
			title = title,
			desc = info,
			author = playerName,
	...
	TriggerClientEvent('mdt:client:newBulletin', -1, src,
		{ id = newBulletin, title = title, info = info, time = time, author = PlayerData.id }, JobType)
end)
		...
		if Config.UsingPsHousing and not Config.UsingDefaultQBApartments then
			...
			for index = 1, #Coords, 1 do
				Houses[#Houses + 1] = {
					...
					coords = tostring(Coords[index]["coords"]["enter"]["x"] ..
						"," .. Coords[index]["coords"]["enter"]["y"] .. "," .. Coords[index]["coords"]["enter"]["z"]),
				}
			end
			person.properties = Houses
		end
...
RegisterNetEvent("mdt:server:saveProfile",
	function(pfp, information, cid, fName, sName, tags, gallery, licenses, fingerprint)
		local src = source
		local xPlayer = vRP.getPlayerInfo(src)
		...
		if xPlayer then
			...
			MySQL.insert.await(
				'INSERT INTO mdt_data (cid, information, pfp, jobtype, tags, gallery, fingerprint) VALUES (:cid, :information, :pfp, :jobtype, :tags, :gallery, :fingerprint) ON DUPLICATE KEY UPDATE cid = :cid, information = :information, pfp = :pfp, jobtype = :jobtype, tags = :tags, gallery = :gallery, fingerprint = :fingerprint',
				{
					cid = cid,
...
RegisterNetEvent('psmdt-mugshot:server:MDTupload', function(citizenid, MugShotURLs)
	...
	local cid = citizenid
	MySQL.insert.await(
		'INSERT INTO mdt_data (cid, pfp, gallery, tags) VALUES (:cid, :pfp, :gallery, :tags) ON DUPLICATE KEY UPDATE cid = :cid,  pfp = :pfp, gallery = :gallery, tags = :tags',
		{
			cid = cid,
		...
		if Player then
			...
			if JobType == 'police' or JobType == 'doj' then
				local matches = MySQL.query.await(
					"SELECT * FROM `mdt_incidents` WHERE `id` LIKE :query OR LOWER(`title`) LIKE :query OR LOWER(`author`) LIKE :query OR LOWER(`details`) LIKE :query OR LOWER(`tags`) LIKE :query OR LOWER(`officersinvolved`) LIKE :query OR LOWER(`civsinvolved`) LIKE :query OR LOWER(`author`) LIKE :query ORDER BY `id` DESC LIMIT 50",
					{
...
RegisterNetEvent('mdt:server:searchBolos', function(sentSearch)
	if sentSearch then
		...
		if JobType == 'police' or JobType == 'ambulance' then
			local matches = MySQL.query.await(
				"SELECT * FROM `mdt_bolos` WHERE `id` LIKE :query OR LOWER(`title`) LIKE :query OR `plate` LIKE :query OR LOWER(`owner`) LIKE :query OR LOWER(`individual`) LIKE :query OR LOWER(`detail`) LIKE :query OR LOWER(`officersinvolved`) LIKE :query OR LOWER(`tags`) LIKE :query OR LOWER(`author`) LIKE :query AND jobtype = :jobtype",
				{
...
RegisterNetEvent('mdt:server:getBoloData', function(sentId)
	if sentId then
		...
		if JobType == 'police' or JobType == 'ambulance' then
			local matches = MySQL.query.await(
				"SELECT * FROM `mdt_bolos` WHERE `id` = :id AND jobtype = :jobtype LIMIT 1", {
					id = sentId,
					jobtype = JobType
				})
...
RegisterNetEvent('mdt:server:newBolo',
	function(existing, id, title, plate, owner, individual, detail, tags, gallery, officersinvolved, time)
		if id then
			...
			local JobType = GetJobType(xPlayer.job.name)
			if JobType == 'police' or JobType == 'ambulance' then
				local fullname = xPlayer.firstname .. ' ' .. xPlayer.lastname

				local function InsertBolo()
					MySQL.insert(
						'INSERT INTO `mdt_bolos` (`title`, `author`, `plate`, `owner`, `individual`, `detail`, `tags`, `gallery`, `officersinvolved`, `time`, `jobtype`) VALUES (:title, :author, :plate, :owner, :individual, :detail, :tags, :gallery, :officersinvolved, :time, :jobtype)',
						{
							title = title,
							author = fullname,
							plate = plate,
							...
							if r then
								...
								TriggerEvent('mdt:server:AddLog',
									"A new BOLO was created by " ..
									fullname .. " with the title (" .. title .. ") and ID (" .. id .. ")")
							end
						end)
				end
				...
				local function UpdateBolo()
					MySQL.update(
						"UPDATE mdt_bolos SET `title`=:title, plate=:plate, owner=:owner, individual=:individual, detail=:detail, tags=:tags, gallery=:gallery, officersinvolved=:officersinvolved WHERE `id`=:id AND jobtype = :jobtype LIMIT 1",
						{
							title = title,
							plate = plate,
							owner = owner,
							...
							if r then
								...
								TriggerEvent('mdt:server:AddLog',
									"A BOLO was updated by " .. fullname .. " with the title (" ..
									title .. ") and ID (" .. id .. ")")
							end
						end)
				end
...
RegisterNetEvent('mdt:server:deleteWeapons', function(id)
	if id then
		...
		if Config.RemoveWeaponsPerms[xPlayer.job.name] then
			if Config.RemoveWeaponsPerms[xPlayer.job.name][xPlayer.job.rank] then
				local fullName = xPlayer.firstname .. ' ' .. xPlayer.lastname
				MySQL.update("DELETE FROM `mdt_weaponinfo` WHERE id=:id", { id = id })
				TriggerEvent('mdt:server:AddLog', "A Weapon Info was deleted by " .. fullName ..
					" with the ID (" .. id .. ")")
			else
				local fullname = xPlayer.firstname .. ' ' .. xPlayer.lastname
...
RegisterNetEvent('mdt:server:deleteReports', function(id)
	if id then
		...
		local xPlayer = vRP.getPlayerInfo(src)
		if not xPlayer then return end
		if Config.RemoveReportPerms[xPlayer.job.name] then
			if Config.RemoveReportPerms[xPlayer.job.name][xPlayer.job.rank] then
				local fullName = xPlayer.firstname .. ' ' .. xPlayer.lastname
...
RegisterNetEvent('mdt:server:deleteIncidents', function(id)
	...
	local xPlayer = vRP.getPlayerInfo(src)
	if not xPlayer then return end
	if Config.RemoveIncidentPerms[xPlayer.job.name] then
		if Config.RemoveIncidentPerms[xPlayer.job.name][xPlayer.job.rank] then
			local fullName = xPlayer.firstname .. ' ' .. xPlayer.lastname
			...
			MySQL.update("DELETE FROM `mdt_incidents` WHERE id=:id", { id = id }, function(rowsChanged)
				if rowsChanged > 0 then
					TriggerEvent('mdt:server:AddLog', "A Incident was deleted by " .. fullName ..
						" with the ID (" .. id .. ")")
				end
			end)
		else
...
RegisterNetEvent('mdt:server:getAllReports', function()
	...
	if xPlayer then
		...
		if JobType == 'police' or JobType == 'doj' or JobType == 'ambulance' then
			if JobType == 'doj' then JobType = 'police' end
			local matches = MySQL.query.await(
				"SELECT * FROM `mdt_reports` WHERE jobtype = :jobtype ORDER BY `id` DESC LIMIT 30", {
					jobtype = JobType
				})
	...
	if sentId then
		...
		if xPlayer then
			...
			if JobType == 'police' or JobType == 'doj' or JobType == 'ambulance' then
				if JobType == 'doj' then JobType = 'police' end
				local matches = MySQL.query.await(
					"SELECT * FROM `mdt_reports` WHERE `id` = :id AND `jobtype` = :jobtype LIMIT 1", {
						id = sentId,
						jobtype = JobType
					})
				local data = matches[1]
	...
	if sentSearch then
		...
		if xPlayer then
			...
			if JobType == 'police' or JobType == 'doj' or JobType == 'ambulance' then
				if JobType == 'doj' then JobType = 'police' end
				local matches = MySQL.query.await(
					"SELECT * FROM `mdt_reports` WHERE `id` LIKE :query OR LOWER(`author`) LIKE :query OR LOWER(`title`) LIKE :query OR LOWER(`type`) LIKE :query OR LOWER(`details`) LIKE :query OR LOWER(`tags`) LIKE :query AND `jobtype` = :jobtype ORDER BY `id` DESC LIMIT 50",
					{
						query = string.lower('%' .. sentSearch .. '%'), -- % wildcard, needed to search for all alike results
						jobtype = JobType
					})
...
RegisterNetEvent('mdt:server:newReport',
	function(existing, id, title, reporttype, details, tags, gallery, officers, civilians, time)
		if id then
			local src = source
			local xPlayer = vRP.getPlayerInfo(src)
			if xPlayer then
				...
				if JobType ~= nil then
					...
					local function InsertReport()
						MySQL.insert(
							'INSERT INTO `mdt_reports` (`title`, `author`, `type`, `details`, `tags`, `gallery`, `officersinvolved`, `civsinvolved`, `time`, `jobtype`) VALUES (:title, :author, :type, :details, :tags, :gallery, :officersinvolved, :civsinvolved, :time, :jobtype)',
							{
								title = title,
								author = fullname,
								type = reporttype,
					...
					local function UpdateReport()
						MySQL.update(
							"UPDATE `mdt_reports` SET `title` = :title, type = :type, details = :details, tags = :tags, gallery = :gallery, officersinvolved = :officersinvolved, civsinvolved = :civsinvolved, jobtype = :jobtype WHERE `id` = :id LIMIT 1",
							{
								title = title,
								type = reporttype,
								details = details,
	...
	if Player then
		...
		if JobType == 'police' or JobType == 'doj' then
			local vehicles = MySQL.query.await(
				"SELECT pv.id, pv.citizenid, pv.plate, pv.vehicle, pv.mods, pv.state, p.charinfo FROM `player_vehicles` pv LEFT JOIN players p ON pv.citizenid = p.citizenid WHERE LOWER(`plate`) LIKE :query OR LOWER(`vehicle`) LIKE :query LIMIT 25",
				{
		...
		if xPlayer then
			...
			if JobType == 'police' or JobType == 'doj' then
				local vehicle = MySQL.single.await(
					"select pv.*, p.charinfo from player_vehicles pv LEFT JOIN players p ON pv.citizenid = p.citizenid where pv.plate = :plate LIMIT 1",
					{ plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
				if vehicle then
					vehicle['impound'] = false
					if vehicle.state == 2 then
						vehicle['impound'] = true
					end

					vehicle['bolo'] = GetBoloStatus(vehicle['plate'])
					vehicle['information'] = ""

					vehicle['name'] = "Unknown Person"

					local ownerResult = json.decode(vehicle.charinfo)
					vehicle['name'] = ownerResult['firstname'] .. " " .. ownerResult['lastname']

					local color1 = json.decode(vehicle.mods)
					vehicle['color1'] = color1['color1']

					vehicle['dbid'] = 0

					local info = GetVehicleInformation(vehicle['plate'])
					if info then
						vehicle['information'] = info['information']
						vehicle['dbid'] = info['id']
						vehicle['points'] = info['points']
						vehicle['image'] = info['image']
						vehicle['code'] = info['code5']
						vehicle['stolen'] = info['stolen']
					end

					if vehicle['image'] == nil then vehicle['image'] = "img/not-found.webp" end
				end
	...
	if plate then
		...
		if xPlayer then
			if GetJobType(xPlayer.job.name) == 'police' then
				...
				local fullname = xPlayer.firstname .. ' ' .. xPlayer.lastname
				TriggerEvent('mdt:server:AddLog',
					"A vehicle with the plate (" .. plate .. ") has a new image (" .. imageurl ..
					") edited by " .. fullname)
				if tonumber(dbid) == 0 then
					MySQL.insert(
						'INSERT INTO `mdt_vehicleinfo` (`plate`, `information`, `image`, `code5`, `stolen`, `points`) VALUES (:plate, :information, :image, :code5, :stolen, :points)',
						{
							plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"),
							information = notes,
							image = imageurl,
							...
							if infoResult then
								...
								TriggerEvent('mdt:server:AddLog',
									"A vehicle with the plate (" ..
									plate .. ") was added to the vehicle information database by " .. fullname)
							end
						end)
				elseif tonumber(dbid) > 0 then
					MySQL.update(
						"UPDATE mdt_vehicleinfo SET `information`= :information, `image`= :image, `code5`= :code5, `stolen`= :stolen, `points`= :points WHERE `plate`= :plate LIMIT 1",
						{
							plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"),
							information = notes,
							image = imageurl,
				...
				if impoundInfo.impoundChanged then
					local vehicle = MySQL.single.await(
						"SELECT p.id, p.plate, i.vehicleid AS impoundid FROM `player_vehicles` p LEFT JOIN `mdt_impound` i ON i.vehicleid = p.id WHERE plate=:plate",
						{ plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
					if impoundInfo.impoundActive then
						local plate, linkedreport, fee, time = impoundInfo['plate'], impoundInfo['linkedreport'],
							impoundInfo['fee'], impoundInfo['time']
						if (plate and linkedreport and fee and time) then
							if vehicle.impoundid == nil then
								...
								local data = vehicle
								MySQL.insert(
									'INSERT INTO `mdt_impound` (`vehicleid`, `linkedreport`, `fee`, `time`) VALUES (:vehicleid, :linkedreport, :fee, :time)',
									{
										vehicleid = data['id'],
										...
										local data = {
											...
											beingcollected = 0,
											vehicle = sentVehicle,
											officer = xPlayer.firstname ..
												" " .. xPlayer.lastname,
											number = xPlayer.phone,
											time = os.time() * 1000,
											src = src,
										}
						...
						if vehicle.impoundid ~= nil then
							...
							local result = MySQL.single.await(
								"SELECT id, vehicle, fuel, engine, body FROM `player_vehicles` WHERE plate=:plate LIMIT 1",
								{ plate = string.gsub(plate, "^%s*(.-)%s*$", "%1") })
							if result then
								local data = result
								MySQL.update("DELETE FROM `mdt_impound` WHERE vehicleid=:vehicleid",
									{ vehicleid = data['id'] })
...
lib.callback.register('mdt:server:SearchWeapons', function(source, sentData)
	...
	if xPlayer then
		...
		if JobType == 'police' or JobType == 'doj' then
			local matches = MySQL.query.await(
				'SELECT * FROM mdt_weaponinfo WHERE LOWER(`serial`) LIKE :query OR LOWER(`weapModel`) LIKE :query OR LOWER(`owner`) LIKE :query LIMIT 25',
				{
					query = string.lower('%' .. sentData .. '%')
				})
			return matches
		...
		if xPlayer then
			...
			if JobType == 'police' or JobType == 'doj' then
				...
				result = MySQL.insert.await(
					'INSERT INTO mdt_weaponinfo (serial, owner, information, weapClass, weapModel, image) VALUES (:serial, :owner, :notes, :weapClass, :weapModel, :imageurl) ON DUPLICATE KEY UPDATE owner = :owner, information = :notes, weapClass = :weapClass, weapModel = :weapModel, image = :imageurl',
					{
						['serial'] = serial,
						['owner'] = owner,
						['notes'] = notes,
				...
				if result then
					TriggerEvent('mdt:server:AddLog',
						"A weapon with the serial number (" ..
						serial .. ") was added to the weapon information database by " .. fullname)
				else
					TriggerEvent('mdt:server:AddLog',
						"A weapon with the serial number (" ..
						serial .. ") failed to be added to the weapon information database by " .. fullname)
				end
			end
		end
	end
...
function CreateWeaponInfo(serial, imageurl, notes, owner, weapClass, weapModel)
	local results = MySQL.single.await('SELECT * FROM mdt_weaponinfo WHERE serial = ?', { serial })
	if results then
		return
	end
...
RegisterNetEvent('mdt:server:getAllLogs', function()
	...
	if Player then
		if Config.LogPerms[xPlayer.job.name] then
			if Config.LogPerms[xPlayer.job.name][xPlayer.job.rank] then
				local JobType = GetJobType(xPlayer.job.name)
				local infoResult = MySQL.query.await(
					'SELECT * FROM mdt_logs WHERE `jobtype` = :jobtype ORDER BY `id` DESC LIMIT 250',
					{ jobtype = JobType })
...
RegisterNetEvent('mdt:server:setCallsign', function(cid, newcallsign)
	local user_id = vRP.getUserIdByCharacterId(cid) -- QBCore.Functions.GetPlayerByCitizenId(cid)
	if user_id then
		vRP.setUserMetadata('callsign', newcallsign)
	end
end)

RegisterNetEvent('mdt:server:saveIncident',
		...
		if xPlayer then
			if GetJobType(xPlayer.job.name) == 'police' then
				if id == 0 then
					local fullname = xPlayer.firstname .. ' ' .. xPlayer.lastname
					MySQL.insert(
						'INSERT INTO `mdt_incidents` (`author`, `title`, `details`, `tags`, `officersinvolved`, `civsinvolved`, `evidence`, `time`, `jobtype`) VALUES (:author, :title, :details, :tags, :officersinvolved, :civsinvolved, :evidence, :time, :jobtype)',
						{
							author = fullname,
							title = title,
							details = information,
							...
							if infoResult then
								MySQL.query(
									'SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id',
									{ ['@id'] = infoResult }, function(result)
										if result and #result > 0 then
											...
											for i = 1, #associated do
												...
												sendIncidentToDiscord(3989503, "MDT Incident Report", message,
													"ps-mdt | Made by Project Sloth", associatedData)
											end
										else
								...
								for i = 1, #associated do
									MySQL.insert(
										'INSERT INTO `mdt_convictions` (`cid`, `linkedincident`, `warrant`, `guilty`, `processed`, `associated`, `charges`, `fine`, `sentence`, `recfine`, `recsentence`, `time`) VALUES (:cid, :linkedincident, :warrant, :guilty, :processed, :associated, :charges, :fine, :sentence, :recfine, :recsentence, :time)',
										{
											cid = associated[i]['Cid'],
				...
				elseif id > 0 then
					MySQL.query('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id',
						{ ['@id'] = id }, function(result)
							if result and #result > 0 then
								...
								for i = 1, #associated do
									...
									sendIncidentToDiscord(16711680, "MDT Incident Report has been Updated", message,
										"ps-mdt | Made by Project Sloth", associatedData)
								end
							else
					...
					MySQL.update(
						'UPDATE `mdt_incidents` SET `title` = @title, `details` = @details, `tags` = @tags, `officersinvolved` = @officersinvolved, `civsinvolved` = @civsinvolved, `evidence` = @evidence, `time` = @time WHERE `id` = @id',
						{
							['@id'] = id,
							['@title'] = title,
							['@details'] = information,
...
RegisterNetEvent('mdt:server:handleExistingConvictions', function(data, incidentId, time)
	MySQL.single('SELECT * FROM mdt_convictions WHERE cid = ? AND linkedincident= ?', {
		cid = data['Cid'],
		linkedincident = incidentId
	}, function(convictionRes)
		if convictionRes then
			MySQL.update(
				'UPDATE mdt_convictions SET cid=:cid, linkedincident=:linkedincident, warrant=:warrant, guilty=:guilty, processed=:processed, associated=:associated, charges=:charges, fine=:fine, sentence=:sentence, recfine=:recfine, recsentence=:recsentence WHERE cid=:cid AND linkedincident=:linkedincident',
				{
					cid = data['Cid'],
					linkedincident = incidentId,
...
RegisterNetEvent('mdt:server:removeIncidentCriminal', function(cid, incident)
	MySQL.update('DELETE FROM mdt_convictions WHERE cid= ? AND linkedincident= ?', {
		cid,
		incident
	})
end)
...
RegisterNetEvent('mdt:server:callDragAttach', function(callid, cid)
	...
	local playerdata = {
		...
		cid = xPlayer.id,
		callsign = xPlayer.datatable?.callsign
	}
	local callid = tonumber(callid)
	local JobType = GetJobType(xPlayer.job.name)
	if JobType == 'police' or JobType == 'ambulance' then
...
RegisterNetEvent('mdt:server:setWaypoint:unit', function(cid)
	local src = source
	local cSource = vRP.getSourceByCharacterId(cid)
	if cSource then
		local PlayerCoords = GetEntityCoords(GetPlayerPed(cSource))
...
RegisterNetEvent('mdt:server:sendMessage', function(message, time)
	if message and time then
		...
		local xPlayer = vRP.getPlayerInfo(src)
		if xPlayer then
			MySQL.scalar("SELECT pfp FROM `mdt_data` WHERE cid= ? LIMIT 1", {
				id = xPlayer.id -- % wildcard, needed to search for all alike results
			}, function(data)
				if data == "" then data = nil end
				local ProfilePicture = ProfPic(xPlayer.gender, data)
				local callsign = xPlayer.datatable.callsign or "000"
				local Item = {
					profilepic = ProfilePicture,
					callsign = callsign,
					cid = xPlayer.id,
					name = '(' .. callsign .. ') ' .. xPlayer.firstname .. " " .. xPlayer.lastname,
...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	local src = source
	local targetPlayer = vRP.getPlayerId(vRP.getUserSource(vRP.getUserIdByPlayerId(cid)))
	local targetSource = targetPlayer.source
	local targetName = targetPlayer.firstname .. ' ' .. targetPlayer.lastname
	local radio = exports.ox_inventory:GetItem(targetSource, 'radio', nil, true) -- targetPlayer.Functions.GetItemByName("radio")
	if radio > 0 then
		TriggerClientEvent('mdt:client:setRadio', targetSource, newRadio)
	else
		vRP.notify(src, 'MDT', targetName .. ' does not have a radio!', 5000, 'error')
	end
end)
			...
			if sentInfo and type(sentInfo) == 'table' then
				...
				if (plate and linkedreport and fee and time) then
					local vehicleId = MySQL.scalar.await(
						...
						{ string.gsub(plate, "^%s*(.-)%s*$", "%1") })
					if vehicleId then
						MySQL.insert(
							'INSERT INTO `mdt_impound` (`vehicleid`, `linkedreport`, `fee`, `time`) VALUES (?, ?, ?, ?)',
							{
								vehicleId,
								linkedreport,
								fee,
								...
								local data = {
									...
									beingcollected = 0,
									vehicle = sentVehicle,
									officer = xPlayer.firstname .. ' ' .. xPlayer.lastname,
									number = xPlayer.phone,
									time = os.time() * 1000,
									src = src,
								}
...
RegisterNetEvent('mdt:server:removeImpound', function(plate, currentSelection)
	...
	local src = source
	local xPlayer = vRP.getPlayerInfo(src) -- vRP.getPlayerInfo( src )
	if xPlayer then
		if GetJobType(xPlayer.job.name) == 'police' then
			local vehicleId = MySQL.single.await("SELECT id FROM `player_vehicles` WHERE plate= ? LIMIT 1",
				{ string.gsub(plate, "^%s*(.-)%s*$", "%1") })
			if vehicleId then
				MySQL.update("DELETE FROM `mdt_impound` WHERE vehicleid = ?", { vehicleId })
				TriggerClientEvent('police:client:TakeOutImpound', src, currentSelection) -- TODO CHANGE QB-CORE EVENT
			end
		end
	end
end)
...
RegisterNetEvent('mdt:server:statusImpound', function(plate)
	...
	if xPlayer then
		if GetJobType(xPlayer.job.name) == 'police' then
			local vehicleId = MySQL.scalar.await("SELECT id FROM `player_vehicles` WHERE plate= ? LIMIT 1",
				{ string.gsub(plate, "^%s*(.-)%s*$", "%1") })
			if vehicleId then
				local impoundinfo = MySQL.single.await("SELECT * FROM `mdt_impound` WHERE vehicleid = ? LIMIT 1",
					{ vehicleId })
				if impoundinfo then
					TriggerClientEvent('mdt:client:statusImpound', src, impoundinfo, plate)
				end
			end
		end
	end
...
function GetBoloStatus(plate)
	local result = MySQL.query.await("SELECT * FROM mdt_bolos where plate = ?", { plate })
	if result then
		local title = result['title']
		local boloId = result['id']
...
function GetWarrantStatus(plate)
	local result = MySQL.single.await(
		[[
SELECT m.id AS `id`, CONCAT(vp.firstname, ' ', vp.lastname) AS `owner`
FROM player_vehicles p
INNER JOIN mdt_convictions m ON p.player_id = m.cid
INNER JOIN vplayers vp ON vp.id = p.player_id
WHERE m.warrant =1 AND p.plate = '0004AA00'
]], { plate })

	if result then
		return true, result.owner, result.id
	end
	return false
end

function GetVehicleInformation(plate)
	return MySQL.single.await('SELECT * FROM mdt_vehicleinfo WHERE plate = ?', { plate })
end

function GetVehicleOwner(plate)
	local result = MySQL.scalar.await(
		[[
SELECT CONCAT(vv.firstname, ' ', vv.lastname) as fullname FROM player_vehicles v
INNER JOIN vplayers vv ON v.player_id = vv.id
WHERE plate = ?
]], { plate })
...
lib.callback.register('mdt:server:GetPlayerSourceId', function(source, cb, targetCitizenId)
...
lib.callback.register('getWeaponInfo', function(source, cb)
	local Player = vRP.getPlayerInfo(source)
	local weaponInfos = {}
	if Config.InventoryForWeaponsImages == "ox_inventory" then
		...
		for _, item in pairs(Player.PlayerData.items) do
			if item.type == "weapon" then
				local invImage = ("https://cfx-nui-%s/html/images/%s"):format(Config.InventoryForWeaponsImages,
					item.image)
				if invImage then
					local weaponInfo = {
...
local function giveCitationItem(src, citizenId, fine, incidentId)
	...
	local Officer = vRP.getPlayerInfo( src )
	local OfficerFullName = '(' ..
		Officer.PlayerData.datatable.callsign ..
	...
	TriggerClientEvent('inventory:client:ItemBox', xPlayer.source, QBCore.Shared.Items['mdtcitation'], "add")
	TriggerEvent('mdt:server:AddLog',
		"A Fine was writen by " ..
		OfficerFullName .. " and was sent to " .. PlayerName .. ", the Amount was $" ..
		fine .. ". (ID: " .. incidentId .. ")")
end
...
RegisterNetEvent('mdt:server:removeMoney', function(citizenId, fine, incidentId)
	...
	if not antiSpam then
		if Player.Functions.RemoveMoney('bank', fine, 'lspd-fine') then
			TriggerClientEvent('QBCore:Notify', xPlayer.source, fine .. "$ was removed from your bank!")
			giveCitationItem(src, citizenId, fine, incidentId)
		else
			TriggerClientEvent('QBCore:Notify', xPlayer.source, "Something went wrong!")
		end
		antiSpam = true
...
function sendToDiscord(color, name, message, footer)
	if ClockinWebhook == '' then
		...
		PerformHttpRequest(ClockinWebhook, function(err, text, headers) end, 'POST',
...
function sendIncidentToDiscord(color, name, message, footer, associatedData)
	...
	if IncidentWebhook == '' then
		...
		PerformHttpRequest(IncidentWebhook, function(err, text, headers) end, 'POST',
			json.encode({ content = pingMessage, username = name, embeds = embed }),
...
if Config.InventoryForWeaponsImages == "ox_inventory" and Config.RegisterWeaponsAutomatically then
	exports.ox_inventory:registerHook('buyItem', function(payload)
		...
		CreateThread(function()
			local owner = QBCore.Functions.GetPlayer(payload.source).PlayerData.id
	...
	if Config.RegisterCreatedWeapons then
		exports.ox_inventory:registerHook('createItem', function(payload)
			...
			CreateThread(function()
				local owner = QBCore.Functions.GetPlayer(payload.inventoryId).PlayerData.id
				if not owner or not payload.metadata.serial then return end
				...
				local success, result = pcall(function()
					return CreateWeaponInfo(payload.metadata.serial, imageurl, notes, owner, weapClass, payload.item
						.name)
				end)
