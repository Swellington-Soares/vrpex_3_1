...
local PlayerData = vRP.getPlayer() or {}
local CurrentCops = 0
local isOpen = false
local callSign = ""
local tabletObj = nil
...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    lib.callback("mdt:server:getWarrants", false, function(data)
        if data then
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    -- local grade = PlayerData.job.rankName

    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Welcome, " ..PlayerData.job.rankName..' '..PlayerData.lastname:sub(1,1):upper()..PlayerData.lastname:sub(2), location = playerStreetsLocation, fullname = PlayerData.firstname..' '..PlayerData.lastname, bulletin = bulletin })
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    lib.callback('mdt:server:SearchProfile', false, function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    local getProfileDataPromise = function(data)
        ...
        p = promise.new()
        lib.callback('mdt:server:GetProfileData', false, function(result)
            p:resolve(result)
        end, data)
    ...
    for i=1,#vehicles do
        ...
        local vehData = nil ---QBCore.Shared.Vehicles[vehicle['vehicle']]
...
RegisterNUICallback("sendFine", function(data, cb)
    ...
    local p = promise.new()
    lib.callback('mdt:server:GetPlayerSourceId', false, function(result)
        p:resolve(result)
    end, citizenId)
    ...
    if not isLimited then
        ...
        for i=1, #vehicles do
            ...
            if tempModel and tempModel ~= "Unknown" then
                local vehData = nil --QBCore.Shared.Vehicles[tempModel]
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    lib.callback('mdt:server:SearchVehicles', false, function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("searchWeapons", function(data, cb)
    ...
    lib.callback('mdt:server:SearchWeapons', false, function(result)
        p:resolve(result)
    end, data.name)
...
function GetPlayerWeaponInfos(cb)
    lib.callback('getWeaponInfo', false, function(weaponInfos)
        cb(weaponInfos)
    end)
end
