...
function Property:UpdateOwner(data)
    ...
    local bank = vRP.getBankMoney( targetPlayer.user_id ) or 0
    ...
    vRP.removeMoney(targetPlayer.user_id, self.propertyData.price, 'bank', "Bought Property: " .. self.propertyData.street .. " " .. self.property_id, false)
    ...
    MySQL.update("UPDATE properties SET owner_id = @owner_id, for_sale = @for_sale WHERE property_id = @property_id", {
        ["@owner_id"] = citizenid,
        ["@for_sale"] = 0,
        ["@property_id"] = self.property_id
    })
