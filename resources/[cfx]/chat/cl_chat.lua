...
AddEventHandler('chat:disable', function(state)
  disabled = state
end)
    ...
    if GetResourceState(resource) == 'started' then
      ...
      if numThemes > 0 then
        ...
        if themeName and themeData then
          themeData.baseUrl = 'https://cfx-nui-' .. resource .. '/'
...
if not isRDR then
  if RegisterKeyMapping then
    RegisterKeyMapping('toggleChat', 'Toggle chat', 'keyboard', 'o')
  end
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if chatLoaded then
      local forceHide = IsScreenFadedOut() or IsPauseMenuActive() or disabled
