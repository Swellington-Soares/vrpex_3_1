...
local function addTemplate(id, html)
	templates[id] = html
end
...
AddEventHandler('chat:addTemplate', function(id, html)
	if not templates[id] then
		addTemplate(id, html)
		TriggerClientEvent('chat:addTemplate', -1, id, html)
	end
end)

exports('addTemplate', function(id, html)
	TriggerEvent('chat:addTemplate', id, html)
end)
...
exports('disable', function(state, player)
	player = player or - 1
	state = state == true
	TriggerClientEvent('chat:disable', player, state)
end)
...
exports('refreshMode', function(modename, player)
	if not modes[modename] then return end
	TriggerClientEvent('chat:addMode', player, modes[modename])
end)
...
local function refreshTemplates(player)
	for k, v in next, templates do
		TriggerClientEvent('chat:addTemplate', player, k, v)
	end
end
